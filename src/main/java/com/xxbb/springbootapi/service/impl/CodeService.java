...
public class CodeService implements ICodeGenerate {
    ...
    public Boolean generate(CodeInput codeInput) {
        ...
        authorityService.add(new Authority().setName(codeInput.getEntityName().toLowerCase()+"-edit").setValue("sys:"+codeInput.getEntityName().toLowerCase()+":edit").setIcon(null).setComponent(null).setPath(null).setTitle(codeInput.getEntityName()+"-编辑").setParentId(authorityTowRouter.getId()).setAuthorityType(AuthorityType.API));
