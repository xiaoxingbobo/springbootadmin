...
import com.xxbb.springbootapi.service.impl.OrderService;
...
@Api(tags = "Order表")
@RestController
public class OrderController extends ApiController<Order, OrderQuery, OrderUpdate, OrderMapper> {
    @Autowired
    ...
    @Override
    @PostMapping
    @PreAuthorize("@auth.hasAuth('sys:order:add')")
    @ApiOperation(value = "添加", notes = "id，创建时间，修改时间无需提交")
    public Boolean create(@RequestBody Order entity) {
    ...
    @Override
    @DeleteMapping("/{id}")
    @PreAuthorize("@auth.hasAuth('sys:order:delete')")
    ...
    @Override
    @PutMapping
    @PreAuthorize("@auth.hasAuth('sys:order:update')")
    ...
    @PreAuthorize("@auth.hasAuth('sys:order:select')")
    @ApiOperation(value = "搜索")
    public List<Order> select(@RequestBody List<Search> searches) {
        if (searches.size() == 1) {
    ...
    public Order find(@PathVariable("id") int id) {
        return (Order) service.find(id);
    }
    ...
    public List<Order> select(@PathVariable("limit") int limit) {
    ...
    @PreAuthorize("@auth.hasAuth('sys:order:select')")
    public PagedResult<Order> select(@RequestBody PagedInputC<Order> input) {
    ...
    public PagedResult<Order> paged(@PathVariable("current") int current, @PathVariable("size") int size) {
    ...
    @PreAuthorize("@auth.hasAuth('sys:order:select')")
    public PagedResult<Order> search(@RequestBody PagedInputC<List<Search>> pagedInput) {
