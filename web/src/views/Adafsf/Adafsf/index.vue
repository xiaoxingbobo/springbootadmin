...
import { Adafsf } from '@/api/adafsf/types'
...
// 添加的实体
const adafsfPayload: Adafsf = reactive({
  id: null,
  createTime: null,
  updateTime: null,
  testa: null,
...
const initData = async () => {
  ...
  const { data: res } = await pagedQueryAdafsf(pagedPlayload)
  res.data.forEach((e) => {
...
const clickEdit = async (row) => {
  adafsfPayload.id = row.id //设置id
  const { data: res } = await getAdafsf(row.id)
  dialog.value = {
    title: '编辑',
    visiable: true
  }
  setValue(adafsfPayload, res)
}
...
const clickDelete = async (row) => {
  ...
  // 点击了确定
  if (res === 'confirm') {
    const res: any = await deleteAdafsf(row.id)
    ElMessage({
      message: res.message,
      type: 'success'
    })
...
const clickSave = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      // 表单验证通过
      if (adafsfPayload.id === null) {
        await addAdafsf(adafsfPayload)
      } else {
        // 编辑
        await putAdafsf(adafsfPayload)
      }
      ElMessage({
        message: '操作成功!',
        type: 'success'
...
const pagedSearches = async (data) => {
  const { data: res } = await pagedSearchAdafsf(data)
...
<template>
  <ContentWrap>
    <div class="mb-10px">
      <ElButton type="success" v-hasPermission="['sys:adafsf:add']" @click="clickAdd">
        <Icon icon="material-symbols:add" />添加
      </ElButton>
      <ElButton v-hasPermission="['sys:adafsf:delete']" type="danger">
    ...
    <div class="mb-10px">
      <el-form :inline="true" label-width="55px">
        ...
        <el-form-item>
          <ElButton type="primary" v-hasPermission="['sys:adafsf:select']" @click="clickQuery">
    ...
    <Table
      ...
      :pagination="tableData.total"
    >
      <template #action="{ row }">
        <ElButton type="danger" v-hasPermission="['sys:adafsf:delete']" @click="clickDelete(row)">
          删除
        </ElButton>
        <ElButton type="primary" v-hasPermission="['sys:adafsf:update']" @click="clickEdit(row)">
          编辑
        </ElButton>
      </template>
    </Table>
  ...
  >
    <!-- 表单 -->
    <el-form ref="diaLogForm" :model="adafsfPayload" label-width="65px">
      <el-form-item label="阿斯顿" prop="testa">
        <el-input v-model="adafsfPayload.testa" autocomplete="off" />
      </el-form-item>
      <el-form-item label="阿三士大夫" prop="asdhoi">
        <el-input v-model="adafsfPayload.asdhoi" autocomplete="off" />
      </el-form-item>
    </el-form>
    <template #footer>
